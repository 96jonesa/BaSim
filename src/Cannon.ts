...
interface TargetableNPC {
    npc: RunnerPenance | HealerPenance;
    currentZone: [number, number];
    zoneCounter: number;
}

export class Cannon {
    public queue: Array<CannonCommand> = [];
    private npcZones: Map<number, { zone: [number, number]; counter: number }> = new Map();
    private npcZonesHealer: Map<number, { zone: [number, number]; counter: number }> = new Map();

    private updateZonePriority(npcs: Array<RunnerPenance | HealerPenance>, zoneMap: Map<number, { zone: [number, number]; counter: number }>): void {
        for (const npc of npcs) {
            const newZone: [number, number] = [npc.position.x >>> 3, npc.position.y >>> 3];
    ...
    public getZoneInfo(npc: RunnerPenance | HealerPenance): { zone: [number, number]; counter: number } {
    ...
    private getTarget(cmd: CannonCommand, barbarianAssault: BarbarianAssault): RunnerPenance | HealerPenance | null {
        ...
        let candidates: Array<{ npc: RunnerPenance | HealerPenance; dist: number; zone: [number, number]; zoneCounter: number }>;

        if (cmd.penance === PenanceType.RUNNER) {
            candidates = barbarianAssault.runners
                .filter((r: RunnerPenance) => {
                    if (r.isDying) return false;
                    if (r.blueCounter >= 0) return false;
                    const dist = cannonPos.distance(r.position);
                    return dist <= CANNON_RANGE;
                })
                .map((r: RunnerPenance) => {
                    const zd = zoneMap.get(r.id) || { zone: [r.position.x >>> 3, r.position.y >>> 3] as [number, number], counter: 1 };
                    return {
                        npc: r,
            ...
            candidates = barbarianAssault.healers
                .filter((h: HealerPenance) => {
                    if (h.isDying) return false;
                    if (h.blueCounter >= 0) return false;
                    const dist = cannonPos.distance(h.position);
                    return dist <= CANNON_RANGE;
                })
                .map((h: HealerPenance) => {
                    const zd = zoneMap.get(h.id) || { zone: [h.position.x >>> 3, h.position.y >>> 3] as [number, number], counter: 1 };
                    return {
                        npc: h,
    ...
    private calculateTravelTime(cannonPos: Position, targetPos: Position, eggType: EggType): number {
        const dist = cannonPos.distance(targetPos);

        if (dist > CANNON_RANGE) return -1;
    ...
    public tick(barbarianAssault: BarbarianAssault): void {
        this.updateZonePriority(barbarianAssault.runners as Array<RunnerPenance | HealerPenance>, this.npcZones);
        this.updateZonePriority(barbarianAssault.healers as Array<RunnerPenance | HealerPenance>, this.npcZonesHealer);
...
export function parseCannonInput(input: string, secondsMode: boolean = false): Array<CannonCommand> {
    ...
    let lastCannon: CannonSide = CannonSide.WEST;
    let lastPenance: PenanceType = PenanceType.RUNNER;
    let lastEggType: EggType = EggType.RED;
    ...
    for (const part of parts) {
        ...
        function parseTick(value: string): number {
            if (secondsMode) {
                ...
                if (Math.abs(t - rounded) > 0.001 || rounded < 1) return NaN;
                return rounded;
            }
            return parseInt(value.trim());
        }

        function parseForcedSplashes(value: string): number {
            const v = parseInt(value.trim());
            if (isNaN(v) || v < 0 || !Number.isInteger(v) || String(v) !== value.trim()) return NaN;
            return v;
        }
        ...
        if (tokens.length === 4 || (tokens.length === 3 && firstIsDescriptor)) {
            ...
            if (forcedSplashes !== null && isNaN(forcedSplashes)) return null;
