...
const HTML_TOGGLE_IGNORE_MAX_HEALERS = "toggleignoremaxhealers";
...
const HTML_START_TICK = "starttick";
...
function init() {
    ...
    toggleSimpleFoodButton.onclick = function () {
        ...
        document.getElementById("settingsexportsolar").disabled = !simpleFood;
    };
    toggleSecondsButton = document.getElementById(HTML_TOGGLE_SECONDS);
    toggleSecondsButton.onclick = toggleSecondsOnClick;
    startTickInput = document.getElementById(HTML_START_TICK);
    startTickInput.onkeydown = function (keyboardEvent) {
        if (keyboardEvent.key === " ") {
            keyboardEvent.preventDefault();
        }
    };
    ...
    document.getElementById(HTML_CLEAR_MARKERS).onclick = clearMarkers;
    ...
    document.getElementById("settingsimportsolar").onclick = importSettingsFromSolar;
...
function getControlledPlayerObject() {
    switch (player) {
    ...
    if (isRunning) {
        ...
        if (!seedBlocked && simpleFood && player === "defender") {
            switch (key) {
                ...
                case "t":
                    barbarianAssault.defenderPlayer.clearCodeQueue();
                    barbarianAssault.defenderPlayer.clearPath();
...
function canvasOnMouseDown(mouseEvent) {
    ...
    const xTile = Math.trunc((mouseEvent.clientX - canvasRect.left) / renderer.tileSize) + renderer.tileOffsetX;
...
function exportSettings() {
    const settings = {
        ...
        ignoreMaxHealers: document.getElementById(HTML_TOGGLE_IGNORE_MAX_HEALERS).checked,
        simpleFood: simpleFood,
...
function importSettings() {
    ...
    try {
        ...
        if (s.ignoreMaxHealers !== undefined)
            document.getElementById(HTML_TOGGLE_IGNORE_MAX_HEALERS).checked = s.ignoreMaxHealers;
...
import { solarCommandsToMclovin, solarDefenderCommandsToMclovin, solarHealerSpawnsToMclovin, } from "./SolarInterop.js";
function importSettingsFromSolar() {
    const field = document.getElementById("solarimportfield");
    const input = field.value.trim();
    if (input.length === 0) {
        return;
    }
    try {
        ...
        if (s.team !== undefined) {
            if (s.team.main !== undefined)
                document.getElementById(HTML_MAIN_ATTACKER_COMMANDS).value = solarCommandsToMclovin(s.team.main);
            if (s.team.second !== undefined)
                document.getElementById(HTML_SECOND_ATTACKER_COMMANDS).value = solarCommandsToMclovin(s.team.second);
            if (s.team.heal !== undefined)
                document.getElementById(HTML_HEALER_COMMANDS).value = solarCommandsToMclovin(s.team.heal);
            if (s.team.col !== undefined)
                document.getElementById(HTML_COLLECTOR_COMMANDS).value = solarCommandsToMclovin(s.team.col);
            if (s.team.def !== undefined)
                document.getElementById(HTML_DEFENDER_COMMANDS).value = solarDefenderCommandsToMclovin(s.team.def);
        }
        field.value = "";
...
function convertDefenderCommands(toSimple) {
    const normalToSimple = { "t": "r", "c": "w", "r": "t" };
    const simpleToNormal = { "r": "t", "w": "c", "t": "r" };
    const map = toSimple ? normalToSimple : simpleToNormal;
    function convertCommandText(text) {
    ...
    const defenderTextarea = document.getElementById(HTML_DEFENDER_COMMANDS);
    ...
    for (let i = 0; i < commands.length; i++) {
        ...
        if (commandTokens.length === 1) {
            ...
            if (commandTokens[0] === "e" && player === "defender") {
                const positionX = Number(commandTokens[1]);
                const positionY = Number(commandTokens[2]);
                if (!Number.isInteger(positionX) || !Number.isInteger(positionY)) {
                    return null;
                }
