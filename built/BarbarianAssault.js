...
export class BarbarianAssault {
    constructor(wave, requireRepairs, requireLogs, infiniteFood, runnerMovements, defenderLevel, mainAttackerCommands, secondAttackerCommands, healerCommands, collectorCommands, defenderCommands, foodCalls, cannonQueue = []) {
        ...
        this.allRunnersDeadTick = null;
        this.healersToRemove = [];
        this.healers = [];
        this.healersAlive = 0;
        this.healersKilled = 0;
        this.allHealersDeadTick = null;
        ...
        this.playerTickStartPositions = new Map();
        this.runnerTickStartPositions = new Map();
        this.runnerSpawns = [];
     ...
     * unless the player also moved during the window.
     */
    checkPhasing(prePositions, didMove) {
        const players = this.allPlayers();
        for (const player of players) {
            if (didMove(player)) {
                player.phased = false;
    ...
    applyJogreBonesForPlayer(player) {
    ...
    executePlayerCommands() {
        if (this.mainAttackerCommands.has(this.ticks)) {
            ...
            commands.forEach((command) => {
                ...
                if (blocked && !(command instanceof WalkRunCommand) && !(command instanceof ToggleRunCommand))
                    return;
    ...
    applyRedEggSplashes() {
        ...
        for (const { target, egg } of arrivingRedEggs) {
            const candidates = allPenance.filter(p => {
                if (p === target)
                    return false;
                if (p.blueCounter >= 0)
                    return false;
            ...
            candidates.sort((a, b) => {
                ...
                if (za.zone[1] !== zb.zone[1])
                    return zb.zone[1] - za.zone[1];
                if (za.zone[0] !== zb.zone[0])
                    return zb.zone[0] - za.zone[0];
                if (za.counter !== zb.counter)
                    return za.counter - zb.counter;
